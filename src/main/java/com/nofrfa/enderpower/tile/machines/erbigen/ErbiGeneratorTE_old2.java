...
import com.nofrfa.enderpower.tile.machines.quantumstorage.QuantumStorage_old;
...
public class ErbiGeneratorTE_old2 extends TileEntityInventory implements INetworkClientTileEntityEventListener, IHasGui, IEnergyStorage {
    ...
    protected void updateEntityServer() {
        ...
        if(!this.connector.isEmpty() && this.connector.get().isItemEqual(new ItemStack(ItemsRegistry.MODULE_binder))) {
            ...
            if (howManyEnergyOutput != 0) {
                ...
                if (tile instanceof QuantumStorage_old) {
                    if(((QuantumStorage_old) tile).getStored() < ((QuantumStorage_old) tile).getCapacity()) {
                        int howManyFreeStorageEnergy = ((QuantumStorage_old) tile).getCapacity() - ((QuantumStorage_old) tile).getStored();
                        if(howManyFreeStorageEnergy < howManyEnergyOutput) {
                            int finalEnergyOutput = howManyEnergyOutput - (howManyEnergyOutput - howManyFreeStorageEnergy);
                            ((QuantumStorage_old) tile).addEnergy(finalEnergyOutput);
                            consumeEnergy(finalEnergyOutput);
                        } else {
                            ((QuantumStorage_old) tile).addEnergy(howManyEnergyOutput);
